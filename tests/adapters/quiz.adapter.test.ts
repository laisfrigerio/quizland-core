...
const id = '812df5b3-5e7c-465e-b65b-db42858e2928';
  (randomUUID as jest.Mock).mockReturnValue('812df5b3-5e7c-465e-b65b-db42858e2928');
...
describe("externalQuizConfigToInternal", () => {
  describe("when level not defined", () => {
    test("should adapter to internal", () => {
      ...
      expect(externalQuizConfigToInternal(overrideConfig))
        .toStrictEqual(mockConfigAdapted);
    });
  });

  describe("when mode defined = training", () => {
    test("should adapter to internal", () => {
      expect(externalQuizConfigToInternal(mockConfig))
        .toStrictEqual(mockConfigAdapted);
    });
  });
  ...
  describe("when mode defined as exam", () => {
    test("should adapter to internal", () => {
      ...
      expect(externalQuizConfigToInternal(overrideConfig))
        .toStrictEqual(overrideConfigAdapted);
    });
  });
});